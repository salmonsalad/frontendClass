...
class KurlyComponent extends Component {
  ...
  idDoubleCheck(value){
    ...
    for(let i=0; i<localStorage.length; i++){
      ...
      if( imsiArr.push( JSON.parse(localStorage.getItem(localStorage.key(i))).아이디 === inpuId )){
        ok=true;
      }      
    }
    
    for(let i=0; i<imsiArr.length; i++){
      if(inpuId===imsiArr[i].아이디){
        console.log( inpuId );
          ...
          return;
            ...
            if(cnt < 3){
              ...
              return;
                  ...
                  const 가입회원정보 = {
                        아이디: 아이디, //문자열 String
                        비밀번호: 비밀번호, //문자열 String
                        이름: 이름, //문자열 String
                        이메일: 이메일, //문자열 String
                        휴대폰: 휴대폰, //문자열 String
                        주소: `${우편번호} ${주소1} ${주소2}`, //문자열 String
                        성별: 성별, //문자열 String
                        생년월일: `${생년} ${생월} ${생일}`, //날짜 Date
                        추가입력사항: 추가입력사항, //문자열 String
                        추가입력상자: 추가입력상자,  //문자열 String
                        약관동의: 약관동의, //문자열 String,
