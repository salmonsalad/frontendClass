(function($){
    ...
    const mega = {
        ...
        section2: function(){
            ...
            //a링크는 앵커버튼 클릭시 화면에 새로고침이 발생
            //화면이 맨위로 올라간다.
            //그런한 기능을 차단
            $('.notice-btn').on({
                click: function(event){
                    alert();
                    event.preventDefault(); //프리벤트디(D)폴트
            ...
            //팝업창(모달창) 투명부분
            //선택자 #popup 를 클릭시 팝압창 닫기(종료) 실행            
            //그리고 닫기 버튼은 닫기가 실행된다.

            //컨테이너 박스 $('.container') : 닫기 실행 안되게 막는다.
            //부모의 버튼 이벤트를 차단해야 가능한다.

            //그러나 팝업창 내용(흰색 배경)은 클릭시 닫기(종료)되면 안된다.
            const popup = $('#popup');
                  ...
                  popup.on({
                      click: function(e){
                        ...
                        if( e.target === e.currentTarget ){
                            ...
                            return false;  //버튼 클릭한 이벤트를 취소한다(리턴)
                        }
                        ...
                        // 현재 클릭한 이벤트의 대상이 e.target
                        // popup 아이디 부모박스의 커렌트 타겟이 아니면 e.currentTarget
                        // 종료 못하게 기능을 차단한다.
                        // 그렇지 않으면 닫기 실행
                        // == 단순 같음(긍정)을 비교 === 논리값(true false) 까지
                        // != 단순 다름(부정)을 비교 !== 논리값까지의 부정의 의미 
                        // console.log('e.target ', e.target ); 
                        // console.log('e.currentTarget ',  e.currentTarget ); 
                        // console.log('e.target === e.currentTarget 긍정문 ',  e.target === e.currentTarget ); 
                        // console.log('e.target !== e.currentTarget 부정문 ',  e.target !== e.currentTarget ); 
